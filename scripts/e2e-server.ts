...
// server per test) and scripts/screenshots.ts. start-server.ts parses the three lines printed below.
...
const server = await startServer({
  ...
  workspacesDir: path.join(state, 'workspaces'),
...
console.log(`workspaces: ${path.join(state, 'workspaces')}`)
